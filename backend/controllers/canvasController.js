...
exports.updateCanvas = async (req, res) => {
    try {
        ...
        if (canvas.owner.toString() !== userId && !canvas.shared.includes(userId)) {
            return res.status(403).json({ error: "Sorry, you are not authorized to update this canvas" });
        }
...
exports.loadCanvas = async (req, res) => {
    try {
        ...
        if (canvas.owner.toString() !== userId && !canvas.shared.includes(userId)) {
            return res.status(403).json({ error: "Sorry, you are not authorized to access this canvas" });
        }
...
exports.shareCanvas = async (req, res) => {
    try {
        ...
        if (canvas.owner.toString() !== userId) {
            console.log("SHARE FAILED: Owner is", canvas.owner.toString(), "but user is", userId);
